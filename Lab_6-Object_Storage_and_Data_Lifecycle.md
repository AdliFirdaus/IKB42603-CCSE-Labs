...
| Bucket used | `miit-patient-records-11787` |
| KMS Key ID | `c26eb03b-2f6b-4e40-bb7e-ef65f9d7be89` |

> **Note on redaction:** All account IDs, key IDs, and access keys shown belong to LocalStack's local dummy identity (account `000000000000`) and are not real AWS credentials, so no redaction was necessary for this report.
...
![get-caller-identity confirms account 000000000000](Evidence-Lab6/setup-06-caller-identity.png)

**Result:** LocalStack Pro running with `ENFORCE_IAM=1`, CLI pointed at it, dummy identity confirmed under account `000000000000`.
...
![Bucket name generated: miit-patient-records-11787](Evidence-Lab6/task1-01-bucket-name.png)
...
![KMS key created (c26eb03b-2f6b-4e40-bb7e-ef65f9d7be89) and set as the bucket's default SSE-KMS key](Evidence-Lab6/task5-01-kms-key-and-encryption-config.png)
...
**Result:** `head-object` returned `ServerSideEncryption: aws:kms`, `SSEKMSKeyId: arn:aws:kms:us-east-1:000000000000:key/c26eb03b-2f6b-4e40-bb7e-ef65f9d7be89`, `BucketKeyEnabled: True` - proof that the bucket-level default control encrypted the object automatically, with no encryption flags supplied at upload time.
...
```
=== IKB42603 Lab 6 verification: miit-patient-records-11787 ===
Public Access Block  -- True True True True
Versioning            -- Enabled
Encryption             -- aws:kms  c26eb03b-2f6b-4e40-bb7e-ef65f9d7be89
